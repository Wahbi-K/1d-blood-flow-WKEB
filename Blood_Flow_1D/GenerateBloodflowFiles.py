...
def generatebloodflowfiles(patient_folder):
    ...
    # generate the boundary file is it does not exist.
    # boundary_file = "boundary_4&21&22&23&24&25&26&30.ply"
    if (not GeneralFunctions.is_non_zero_file(surfacefile)):
        temp_patient = PatientClass.Patient(patient_folder)
        temp_patient.Perfusion.PrimalGraph.LoadSurface(Patient.Folders.ModellingFolder + "labelled_vol_mesh.msh")
        temp_patient.Perfusion.PrimalGraph.File = "PialSurface.vtp"
        temp_patient.Perfusion.PrimalGraph.GraphToVTP(Patient.Folders.ModellingFolder)
        Remesh.remesh(surfacefile,
                      numbertriangles=20000,
    ...
    #
    # if (not GeneralFunctions.is_non_zero_file(surfacefile)) and Patient.ModelParameters["UsingPAfile"] == True:
    #     # remesh the mesh to a uniform triangulation.
    #     print("Pial Surface file not found.")
    #     Remesh.remesh(Patient.Folders.ModellingFolder + boundary_file, numbertriangles=100000,
    #                   output=Patient.Folders.ModellingFolder + "remeshed.vtp")
    #     # apply the same mapping to the remeshed file
    #     Remesh.MapMeshtoMSH(Patient.Folders.ModellingFolder + "remeshed.vtp",
    #                         Patient.Folders.ModellingFolder + "labelled_vol_mesh.msh",
    ...
    if ClusteringCompute == "True":
        ...
        if Patient.ModelParameters['brain_mesh'] == 'default':
            cluster_folder = "/Clustering_Lib/"
        else:
            cluster_folder = "/Clustering_Lib_old/"
        surfacefile = Patient.Folders.DataFilesFolder + cluster_folder + \
                      Patient.ModelParameters["Donor_Network"][:-4] + "/Clustering.vtp"
        ...
        file = Patient.Folders.DataFilesFolder + cluster_folder + \
               Patient.ModelParameters["Donor_Network"][:-4] + "/Clusters.csv"
...
if __name__ == '__main__':
    ...
    if os.path.isfile(patient_folder + "/bf_generation.log"):
        try:
            shutil.rmtree(patient_folder + "/bf_generation.log")
        except:
            print("Error while deleting file ", patient_folder + "/bf_generation.log")
